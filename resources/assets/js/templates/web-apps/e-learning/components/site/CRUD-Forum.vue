<template>
    <v-layout justify-center>
		...
		<v-dialog v-model="dialog" persistent max-width="80%">
			<v-card>
				<v-card-title>
					<span class="headline" style="text-transform: capitalize">{{ operation }} Question</span>
				</v-card-title>
				<v-card-text>
							...
							<v-layout row wrap>
								<v-flex sm7>
									...
									<v-text-field v-model="paragraph" label="Paragraph" :error-messages="errors.collect('paragraph')"
											v-validate="'required'" data-vv-name="paragraph" required multi-line rows="10"></v-text-field>
                                    <small>Supports markdown.</small>
								</v-flex>
								<v-flex offset-sm1 sm3>
									<v-text-field v-for="(tag, i) in tags" :key="i" v-model="tags[i]" label="Tag" data-vv-name="tag"
                                        :error-messages="errors.collect('tag')" v-validate="'required'" required ></v-text-field>
									<v-btn @click="tags.push('')" icon>
                                        <v-icon>add</v-icon>
                                    </v-btn>
									<v-btn @click="tags.pop()" icon>
                                        <v-icon>delete</v-icon>
                                    </v-btn>
								</v-flex>
							</v-layout>
...
<script>
    ...
    export default {
        name: 'CRUD-Forum',
        props: {address: String, id: String, sectionid: String, c: Boolean, u: Boolean, d: Boolean, r: Boolean, parent: Object},
        data() {
            return {
                title: '',
                type: '',
