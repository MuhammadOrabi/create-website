		...
		<v-dialog v-model="dialog" persistent max-width="100%">
			<v-card>
							...
							<v-layout  row wrap>
								<v-flex sm5>
									...
									<v-btn color="primary" v-if="operation == 'update'" 
										:disabled="errors.any() || invalid || loading" @click="update(null)">Update Content</v-btn>
								</v-flex>
								<v-flex sm6 class="text-xs-center" >
									...
									<v-btn color="primary" v-if="operation == 'update'" 
										:disabled="loading" @click="uploadVideoThenDoOperation">Update Video</v-btn>				
									<v-progress-circular v-if="loading" :size="100" :width="15" :rotate="360" :value="progress" color="teal">
										{{ progress }}
									</v-progress-circular>
									<input v-else type="file" style="display: none" ref="videoInput" accept="video/*" @change="onVideoPicked">
				...
				<v-card-actions>
					...
					<v-btn color="primary" v-if="operation == 'create'" :disabled="errors.any() || invalid" 
...
export default {
	...
	methods: {
		...
		uploadVideoThenDoOperation() {
			this.loading = true;
			let date = new Date();
			let fileName = process.env.NODE_ENV + '/' +this.address + '/' + date.toDateString() + '/' + this.videoFile.name;
			let upload = window.firebase.storage().ref(fileName).put(this.videoFile);
		...
		create(video) {
			const vm = this;
			let data = {title: vm.title, paragraph: vm.paragraph, files: vm.files, video: video};
